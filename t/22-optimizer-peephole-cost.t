...
SKIP: {
	...
	{ no warnings 'redefine'; no warnings 'once';
		*RDF::Query::Model::Redland::supports	= sub {
			my $self	= shift;
			my $feature	= shift;
	...
	
	{
		...
		isa_ok( $opt, 'RDF::Query::Optimizer::Peephole::Cost' );
	}
	
	my ($test_rdf, @data)	= YAML::Load(do { local($/) = undef; <DATA> });
	$bridge->add_string( $test_rdf, 'http://example.com/', 0, 'turtle' );
	
	foreach (@data) {
		die "DUPLICATE PARSE TREE for $_->[0]" unless (scalar(@$_) == 5);
		my ($name, $sparql, undef, $expected_cost_parsed, $expected_cost)	= @$_;
		...
		
		{
			...
			die Dumper($query->{'parsed'}{'triples'}) unless
				is_deeply($query->{'parsed'}{'triples'}, $expected_cost_parsed->{'triples'}, "optimized triple pattern order of ${name}");
		}
	}
}
